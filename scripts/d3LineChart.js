...
function drawLineChart(allCovidData) {
  ...
  let x = d3.scaleTime()
    .domain(d3.extent(singlePlaceData, d => new Date(d.Date_of_report.substring(0,10)))) // Taking only the date with substring
    .range([0, width]);
  ...
  svg.append("path")
     ...
     .attr("d", d3.line().curve(d3.curveLinear)
      .x(d => { console.log("X value:", x(new Date(d.Date_of_report.substring(0,10))));
        x(new Date(d.Date_of_report.substring(0,10)))
      })
      .y(d => {
        console.log("Y value:", y(+d.Deceased));
        y(+d.Deceased)
      }));
