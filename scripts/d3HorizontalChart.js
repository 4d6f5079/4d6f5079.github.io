...
function drawPieChart(data) {
    ...
    const sortedData = data.sort((a, b) => d3.descending(getCatWithUndefCheck(a),getCatWithUndefCheck(b)))

    const topData = data
    .map(d => getCatWithUndefCheck(d))
    ...
    const startColor = 
    (selectedCategory === "Covid-19 Infections") ? 
    "#c7bc7b" : (selectedCategory === "Hospital Admissions") ?
    "#9dd1cd" : "#9dd1a0";
    const stopColor = (selectedCategory === "Covid-19 Infections") ? 
    "#8b0000" : (selectedCategory === "Hospital Admissions") ?
    "blue" : "#006400";
    const count = topData.length
    const interpolate = d3.interpolateRgb(startColor, stopColor)
    
    var arc = d3.arc()
        .outerRadius(radius);
    var pie = d3.pie();
    var svg_chart = d3.select("#horizontal-chart").append("svg")
        .datum(topData)
        .attr('class', 'chart')
        .attr("width", width)
        .attr("height", height)
    ...
    var arcs = svg_chart.selectAll("g.arc")
        .data(pie)
        .enter().append("g")
        .attr("class", "arc");
    ...
    arcs.append("path")
        .attr("fill", function(d) {
            return interpolate(d.data/max); 
        })
        // .style("fill-opacity", 0.75)
        .call(d3.helper.tooltip(function(d, i) {
            return tooltipText(sortedData[i]);
        }, true))
    .transition()
        .ease(d3.easeBounce)
        .duration(1500)
    ...
    .transition()
        .ease(d3.easeElastic)
        .delay(function(d, i) { return 1500 + i * 10; })
        .duration(750)
